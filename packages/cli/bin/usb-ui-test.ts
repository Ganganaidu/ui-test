...
import { Logger, LogLevel, type TestResult } from '@usb-ui-test/common';
import { formatResolvedAppSummary } from '../src/appConfig.js';
import { CliEnv, MODEL_FORMAT_EXAMPLE, parseModel, parseModelOptional } from '../src/env.js';
...
program
  ...
  .argument(
    '[selectors...]',
    'Workspace-relative YAML files, directories, or globs under .usb-ui-test/tests',
  )
...
async function runTestCommand(params: {
  ...
  try {
    ...
    const model = parseModelOptional(params.options.model ?? workspaceConfig.model);
    const features = workspaceConfig.features;
    const reasoning = workspaceConfig.reasoning;

    const requiredProviders = new Set<string>();
    if (model) requiredProviders.add(model.provider);
    if (features) {
