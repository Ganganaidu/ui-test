...
function createTempWorkspace(params?: {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-workspace-'));
  ...
  if (configYaml !== undefined) {
    fs.writeFileSync(
      path.join(rootDir, '.usb-ui-test', 'config.yaml'),
      configYaml,
      'utf-8',
    );
  }
...
async function withTempHome<T>(callback: (homeDir: string) => Promise<T>): Promise<T> {
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-workspace-home-'));
...
test('runCheck resolves the nearest .usb-ui-test workspace and runtime bindings from a nested cwd', async () => {
...
test('runCheck uses .usb-ui-test/config.yaml env when --env is omitted', async () => {
  const rootDir = createTempWorkspace({
...
test('runCheck prefers explicit envName over .usb-ui-test/config.yaml env', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'env: staging\n',
    envFiles: {
      'dev.yaml': ['variables:', '  language: Spanish'].join('\n'),
...
test('runCheck succeeds with empty bindings when envName is omitted and .usb-ui-test/env is absent', async () => {
  const rootDir = createTempWorkspace({
    includeEnvDir: false,
    testFiles: {
...
test('runCheck succeeds with empty bindings when envName is omitted and .usb-ui-test/env exists but has no env files', async () => {
  const rootDir = createTempWorkspace({
    envFiles: {},
    testFiles: {
      'smoke.yaml': ['name: smoke', 'steps:', '  - Open the app.'].join('\n'),
...
test('runCheck fails with actionable guidance when a test references env bindings but .usb-ui-test/env is absent', async () => {
  const rootDir = createTempWorkspace({
    includeEnvDir: false,
    testFiles: {
      'language.yaml': [
...
test('runCheck fails with actionable guidance when --env is explicit but .usb-ui-test/env is absent', async () => {
  const rootDir = createTempWorkspace({
    includeEnvDir: false,
  });
  ...
  try {
    await assert.rejects(
      ...
      /Environment "dev" was requested, but .*\/\.usb-ui-test\/env does not exist/,
    );
  } finally {
    await fsp.rm(rootDir, { recursive: true, force: true });
  }
...
test('runCheck preserves the missing-env error when .usb-ui-test/config.yaml points to a missing env file', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'env: staging\n',
    envFiles: {
      'dev.yaml': '{}\n',
...
test('runCheck requires app config in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
    configYaml: null,
  });
  ...
  try {
    await assert.rejects(
      () => runCheck({ cwd: rootDir }),
      /\.usb-ui-test\/config\.yaml must define app\.packageName and\/or app\.bundleId/,
    );
  } finally {
    await fsp.rm(rootDir, { recursive: true, force: true });
  }
...
test('runCheck rejects nested app config and points to the flat schema', async () => {
  ...
  try {
    await assert.rejects(
      () => runCheck({ cwd: rootDir }),
      /\.usb-ui-test\/config\.yaml app uses an unsupported nested format\. Use app\.name, app\.packageName, and app\.bundleId\./,
    );
  } finally {
    await fsp.rm(rootDir, { recursive: true, force: true });
  }
...
test('runCheck rejects empty app identifiers in workspace config', async () => {
  ...
  try {
    await assert.rejects(
      () => runCheck({ cwd: rootDir }),
      /\.usb-ui-test\/config\.yaml app packageName must not be empty\./,
    );
  } finally {
    await fsp.rm(rootDir, { recursive: true, force: true });
  }
...
test('runCheck ignores invalid model formats in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
...
test('runCheck ignores unsupported model providers in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
...
test('runCheck rejects invalid YAML in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'env: [dev\n',
  });
  ...
  try {
    await assert.rejects(
      () => runCheck({ cwd: rootDir }),
      /Invalid YAML in .*\/\.usb-ui-test\/config\.yaml:/,
    );
  } finally {
    await fsp.rm(rootDir, { recursive: true, force: true });
  }
...
test('runCheck rejects unknown keys in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
...
test('runCheck rejects non-string env values in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'env: 123\n',
  });
...
test('runCheck rejects non-string model values in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
...
test('runCheck rejects empty env values in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'env: "   "\n',
  });
...
test('runCheck rejects invalid reasoning level in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
...
test('runCheck rejects unknown feature names in .usb-ui-test/config.yaml', async () => {
  const rootDir = createTempWorkspace({
...
test('runCheck rejects selectors that escape .usb-ui-test/tests', async () => {
  const rootDir = createTempWorkspace();
...
test('validateAppOverride accepts .apk and .app bundles and rejects unsupported paths', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-app-override-'));
...
test('ensureWorkspaceDirectories creates a hashed external artifacts directory and metadata', async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      assert.equal(
        workspace.artifactsDir.startsWith(path.join(homeDir, '.usb-ui-test', 'workspaces') + path.sep),
        true,
      );
...
test('resolveWorkspaceForCommand resolves an explicit workspace path, persists a derived display name, and refreshes lastUsedAt', async () => {
  await withTempHome(async () => {
    const rootDir = createTempWorkspace();
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-outside-workspace-'));
    const existingLastUsedAt = '2020-01-01T00:00:00.000Z';
    fs.writeFileSync(
...
test('resolveWorkspaceForCommand shows the TTY picker, ignores stale and malformed registry entries, and persists runtime-derived labels', async () => {
  await withTempHome(async () => {
    ...
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-picker-outside-'));
    ...
    try {
      ...
      assert.match(outputText, /Select a UsbUiTest workspace/);
...
test('resolveWorkspaceForCommand fails with guidance outside a workspace when no TTY or explicit workspace is available', async () => {
  await withTempHome(async () => {
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-missing-workspace-'));
...
test('resolveWorkspaceFromPath reports a clear error for invalid explicit workspace paths', async () => {
  await withTempHome(async () => {
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-invalid-workspace-'));
