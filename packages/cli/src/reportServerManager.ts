...
export async function stopWorkspaceReportServer(
  ...
  if (livePid === undefined) {
    throw new Error(
      `Could not safely stop the UsbUiTest report server for ${workspace.rootDir} because the live server did not report a pid.`,
    );
  }
  ...
  if (!Number.isInteger(pid) || pid <= 0) {
    throw new Error(
      `Healthy UsbUiTest report server did not report a valid pid for ${workspace.rootDir}.`,
    );
  }
...
export function getWorkspaceReportServerStatePath(workspace: UsbUiTestWorkspace): string {
