...
function createWorkspace(): UsbUiTestWorkspace {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-report-manager-'));
  const usbUiTestDir = path.join(rootDir, '.usb-ui-test');
  const testsDir = path.join(usbUiTestDir, 'tests');
  const suitesDir = path.join(usbUiTestDir, 'suites');
  const envDir = path.join(usbUiTestDir, 'env');
...
function createServerState(workspace: UsbUiTestWorkspace): ReportServerState {
  return {
    pid: 4321,
    port: 4173,
