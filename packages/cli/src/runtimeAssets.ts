...
export class RuntimeAssetStore {
  ...
  async resolveAssetPath(kind: RuntimeAssetKind): Promise<string | null> {
    ...
    if (downloaded.length !== asset.size) {
      throw new Error(
        `Downloaded UsbUiTest asset ${asset.filename} has size ${downloaded.length}, expected ${asset.size}.`,
      );
    }
    if (downloadedHash !== asset.sha256.toLowerCase()) {
      throw new Error(
        `Downloaded UsbUiTest asset ${asset.filename} failed checksum verification.`,
      );
    }
  ...
  private async _loadManifest(): Promise<RuntimeAssetManifestRecord | undefined> {
    ...
    this._manifestPromise = (async () => {
      ...
      if (manifestUrl && manifestUrl.trim()) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to download the UsbUiTest asset manifest from ${manifestUrl}: ${response.status} ${response.statusText}`,
          );
        }
  ...
  private async _readAssetBytes(assetUrl: string): Promise<Buffer> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to download the UsbUiTest asset from ${assetUrl}: ${response.status} ${response.statusText}`,
      );
    }
