...
import { PLATFORM_ANDROID, PLATFORM_IOS } from '@usb-ui-test/common';
import { runDoctorCommand } from './doctorRunner.js';
import { resolveWorkspace } from './workspace.js';

const CLI_TEST_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-home-'));
...
function createTempWorkspace(params?: {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-bin-'));
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  const suitesDir = path.join(rootDir, '.usb-ui-test', 'suites');
  ...
  if (configYaml !== undefined) {
    fs.writeFileSync(path.join(rootDir, '.usb-ui-test', 'config.yaml'), configYaml, 'utf-8');
  }
...
function runCli(args: string[], cwd: string, envOverrides?: NodeJS.ProcessEnv) {
  const compiledBinPath = path.resolve(__dirname, '../bin/usb-ui-test.js');
  const sourceBinPath = path.resolve(__dirname, '../bin/usb-ui-test.ts');
...
test('usb-ui-test check works without --env when dev.yaml exists', async () => {
...
test('usb-ui-test check uses .usb-ui-test/config.yaml env when --env is omitted', async () => {
  const rootDir = createTempWorkspace({
...
test('usb-ui-test check prefers --env over .usb-ui-test/config.yaml env', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'env: staging\n',
    envFiles: {
      'dev.yaml': ['variables:', '  locale: en-US'].join('\n'),
...
test('usb-ui-test check ignores config model values', async () => {
  const rootDir = createTempWorkspace({
...
test('usb-ui-test check reports an env ambiguity error instead of a parser error when --env is omitted', async () => {
  const rootDir = createTempWorkspace({
    envFiles: {
      'staging.yaml': '{}\n',
      'prod.yaml': '{}\n',
...
test('usb-ui-test check works without --env when .usb-ui-test/env is absent and the test is env-free', async () => {
  const rootDir = createTempWorkspace({
    includeEnvDir: false,
  });
...
test('usb-ui-test check fails with actionable binding guidance when .usb-ui-test/env is absent and the test references env bindings', async () => {
  const rootDir = createTempWorkspace({
    includeEnvDir: false,
...
test('usb-ui-test check rejects tests with preconditions keys', async () => {
  const rootDir = createTempWorkspace({
    testLines: [
      'name: login',
      'preconditions:',
...
test('usb-ui-test check accepts repeated selectors and comma-delimited selectors', async () => {
  const rootDir = createTempWorkspace({
    testFiles: {
...
test('usb-ui-test check validates suite manifests with --suite', async () => {
  const rootDir = createTempWorkspace({
    testFiles: {
      'login.yaml': ['name: login', 'steps:', '  - Open the login screen.'].join('\n'),
...
test('usb-ui-test test resolves nested test paths without requiring the .usb-ui-test/tests prefix', async () => {
  const rootDir = createTempWorkspace({
...
test('usb-ui-test suite resolves suite manifests without requiring the .usb-ui-test/suites prefix', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'model: google/gemini-3-flash-preview\n',
    testFiles: {
      'login/auth.yaml': ['name: auth login', 'steps:', '  - Open auth login.'].join('\n'),
...
test('usb-ui-test test rejects --suite flag as an unknown option', async () => {
  const rootDir = createTempWorkspace({
    suites: {
...
test('usb-ui-test test reports missing selectors before API key validation', async () => {
  const rootDir = createTempWorkspace();
...
test('usb-ui-test test requires --model before resolving the workspace environment', async () => {
  const rootDir = createTempWorkspace({
    envFiles: {
      'prod.yaml': '{}\n',
      'staging.yaml': '{}\n',
...
test('usb-ui-test test uses .usb-ui-test/config.yaml model when --model is omitted', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'model: google/gemini-3-flash-preview\n',
  });
...
test('usb-ui-test test validates malformed config models before resolving the workspace environment', async () => {
  const rootDir = createTempWorkspace({
...
test('usb-ui-test test validates unsupported config providers before resolving the workspace environment', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'model: bedrock/claude\n',
    envFiles: {
      'prod.yaml': '{}\n',
...
test('usb-ui-test test uses .usb-ui-test/config.yaml env when --env is omitted', async () => {
  const rootDir = createTempWorkspace({
...
test('usb-ui-test test prefers explicit --model over .usb-ui-test/config.yaml model', async () => {
  const rootDir = createTempWorkspace({
    configYaml: 'model: bedrock/claude\n',
  });
...
test('usb-ui-test test prefers explicit --env over .usb-ui-test/config.yaml env', async () => {
  const rootDir = createTempWorkspace({
...
test('usb-ui-test test rejects unsupported providers before resolving the workspace environment', async () => {
  const rootDir = createTempWorkspace({
    envFiles: {
      'prod.yaml': '{}\n',
      'staging.yaml': '{}\n',
...
test('usb-ui-test test prints blocked Android preflight failures raw and does not create a run', async () => {
  const rootDir = createTempWorkspace();
  ...
  try {
    ...
    assert.match(result.stderr, /usb-ui-test doctor --platform android/);
...
test('usb-ui-test test prints missing test selector failures raw and does not create a run', async () => {
  const rootDir = createTempWorkspace();
...
test('usb-ui-test suite prints missing manifest failures raw and does not create a run', async () => {
  const rootDir = createTempWorkspace({
...
test('usb-ui-test test prints invalid YAML failures raw and does not create a run', async () => {
  const rootDir = createTempWorkspace({
    testFiles: {
...
test('usb-ui-test test prints unresolved env binding failures raw and does not create a run', async () => {
  const rootDir = createTempWorkspace({
    includeEnvDir: false,
    testLines: ['name: login', 'steps:', '  - Enter ${secrets.email} on the login screen.'],
  });
...
test('usb-ui-test test prints unsupported app override failures raw and does not create a run', async () => {
  const rootDir = createTempWorkspace();
...
test('usb-ui-test test prints device setup failures raw and does not create a run', async () => {
  const rootDir = createTempWorkspace();
  ...
  try {
    const result = runCli(
      ...
      rootDir,
      {
        ...
        USB_UI_TEST_CLI_TEST_FORCE_DEVICE_SETUP_FAILURE:
...
test('usb-ui-test start-server reports guidance outside a UsbUiTest repo when --workspace is omitted', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-no-workspace-'));

  try {
    const result = runCli(['start-server'], rootDir);
    assert.equal(result.status, 1);
    assert.match(result.stderr, /Pass --workspace <path> to target a UsbUiTest workspace explicitly/);
  } finally {
    await fsp.rm(rootDir, { recursive: true, force: true });
  }
});

test('usb-ui-test start-server ignores a parent .usb-ui-test home directory without tests', async () => {
  const fakeHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-fake-home-'));
  const outsideDir = path.join(fakeHomeDir, 'projects', 'outside');
  fs.mkdirSync(path.join(fakeHomeDir, '.usb-ui-test'), { recursive: true });
  ...
  try {
    ...
    assert.equal(result.status, 1);
    assert.match(result.stderr, /Pass --workspace <path> to target a UsbUiTest workspace explicitly/);
    assert.doesNotMatch(result.stderr, /Missing \.usb-ui-test\/tests directory/);
  } finally {
    await fsp.rm(fakeHomeDir, { recursive: true, force: true });
  }
});

test('usb-ui-test start-server --help shows --workspace and top-level stop/status commands are available', async () => {
  const rootDir = createTempWorkspace();
  ...
  try {
    ...
    assert.match(stopHelp.stdout, /stop the local usb-ui-test report server/i);
    assert.match(stopHelp.stdout, /--workspace <path>/);
    assert.equal(statusHelp.status, 0);
    assert.match(statusHelp.stdout, /show the local usb-ui-test report server status/i);
...
test('usb-ui-test report serve is rejected after the breaking command removal', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-no-report-workspace-'));
...
test('usb-ui-test runs --json prints the saved runs index', async () => {
  const rootDir = createTempWorkspace();
...
test('usb-ui-test runs prints a console summary and suggests starting the local report UI', async () => {
  const rootDir = createTempWorkspace();
  const workspace = await resolveWorkspaceForHome(rootDir, CLI_TEST_HOME);
  const artifactsDir = workspace.artifactsDir;
  await fsp.mkdir(artifactsDir, { recursive: true });
  ...
  try {
    ...
    assert.match(
      result.stdout,
      /usb-ui-test start-server --workspace "/,
    );
    assert.match(
      result.stdout,
...
test('usb-ui-test start-server rejects malformed and out-of-range --port values', async () => {
  const rootDir = createTempWorkspace();
...
test('usb-ui-test runs --workspace works from outside any workspace', async () => {
  const workspaceRoot = createTempWorkspace();
  const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-outside-runs-'));
...
test('usb-ui-test runs --workspace overrides the current workspace', async () => {
...
test('usb-ui-test runs reports a clear error for an invalid explicit workspace path', async () => {
  const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-invalid-workspace-'));
  ...
  try {
    ...
    assert.equal(result.status, 1);
    assert.match(result.stderr, /Path is not inside a UsbUiTest workspace/);
  } finally {
    await fsp.rm(outsideDir, { recursive: true, force: true });
  }
});

test('usb-ui-test start-server, server-status, and stop-server work from outside a workspace with --workspace', async () => {
  const workspaceRoot = createTempWorkspace();
  const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-server-outside-'));
  ...
  try {
    ...
    assert.match(statusResult.stdout, /UsbUiTest report server status/);
    ...
    assert.match(stopResult.stdout, /Stopped UsbUiTest report server/);
