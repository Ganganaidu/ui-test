...
export class CliFilePathUtil implements FilePathUtil {
  ...
  async ensureIOSAppsAvailable(): Promise<void> {
    ...
    if (!appZipPath || !fs.existsSync(appZipPath)) {
      throw new Error(
        `Missing iOS driver archive in ${iosDir}. Configure USB_UI_TEST_ASSET_DIR, USB_UI_TEST_ASSET_MANIFEST_PATH, or USB_UI_TEST_ASSET_MANIFEST_URL.`,
      );
    }
    if (!runnerZipPath || !fs.existsSync(runnerZipPath)) {
      throw new Error(
        `Missing iOS test runner archive in ${iosDir}. Configure USB_UI_TEST_ASSET_DIR, USB_UI_TEST_ASSET_MANIFEST_PATH, or USB_UI_TEST_ASSET_MANIFEST_URL.`,
      );
    }
