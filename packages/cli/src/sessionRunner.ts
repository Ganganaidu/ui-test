...
} from '@usb-ui-test/goal-executor';
...
import {
  ...
  type StructuredStep,
} from '@usb-ui-test/local-executor';
import type { DeviceLogCaptureResult } from '@usb-ui-test/common';
...
export async function executeTestOnSession(
  ...
  try {
    ...
    if (!config.defaults.provider) {
      throw new Error(
        '--model is required for AI-mode tests. Use provider/model format, for example google/gemini-3-flash-preview. Supported providers: openai, google, anthropic.',
      );
    }
...
async function ensureAppReady(
  ...
  if (!isInstalled) {
    throw new Error(
      `${formatAppReference(app)} is not installed on the selected device. Pass --app <path> to install it or install it manually before running UsbUiTest.`,
    );
  }
...
async function runDeterministicTestOnSession(
  session: TestSession,
  config: TestSessionConfig,
  renderer: GoalRunnerRenderer,
): Promise<TestExecutionResult> {
  const startedAt = new Date().toISOString();
  const steps: AgentActionResultDeterministic[] = [];
  ...
  if (!driver) {
    ...
    const result: TestExecutionResult = {
      ...
      status: 'failure',
      message:
        'Deterministic mode requires a CommonDriverActions-compatible device handle, ' +
  ...
  const executor = new DeterministicExecutor({
    ...
    onProgress: async (event: ExecutorProgressEvent) => {
  ...
  const finalStatus: TestExecutionResult['status'] = runResult.success
    ? 'success'
    : 'failure';
  ...
  const finalResult: TestExecutionResult = {
    ...
    startedAt,
    completedAt,
    steps: steps as unknown as TestExecutionResult['steps'],
...
/**
 * Subset of AgentActionResult populated by the deterministic runner. We don't
 * currently produce screenshots / hierarchies / LLM traces in this path —
 * those fields stay undefined and are tolerated by the existing renderers.
 */
interface AgentActionResultDeterministic {
  iteration: number;
  action: string;
  reason: string;
 ...
 */
function createDriverFromSession(session: TestSession): DriverHandle | null {
  ...
  const device = session.device as unknown as Partial<DriverHandle>;
  if (
