...
export function resolveAppConfig(params: {
  ...
  if (!workspaceApp?.packageName && !workspaceApp?.bundleId) {
    throw new Error(
      '.usb-ui-test/config.yaml must define app.packageName and/or app.bundleId. App config is required for UsbUiTest runs.',
    );
  }
  ...
  if (platform === PLATFORM_ANDROID) {
    if (!effectiveApp.packageName) {
      throw new Error(
        'No app config found for platform "android". Add app.packageName to .usb-ui-test/config.yaml or choose a different --platform.',
      );
    }
    const resolved = {
      platform,
  ...
  if (!effectiveApp.bundleId) {
    throw new Error(
      'No app config found for platform "ios". Add app.bundleId to .usb-ui-test/config.yaml or choose a different --platform.',
    );
  }
  const resolved = {
    platform,
