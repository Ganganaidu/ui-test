...
import { resolveUsbUiTestRootDir } from './runtimePaths.js';
...
export async function resolveWorkspace(
  ...
  throw new Error(
    'Could not find a .usb-ui-test workspace. Run the CLI from a repository containing .usb-ui-test/.',
  );
}
...
export async function resolveWorkspaceFromPath(
  ...
  throw new Error(
    `Path is not inside a UsbUiTest workspace: ${resolvedPath}. Pass --workspace <path> that points to a repository containing .usb-ui-test/.`,
  );
}
...
export async function resolveWorkspaceForCommand(params?: {
  ...
  let workspace: UsbUiTestWorkspace | undefined;
  ...
  if (!workspace) {
    throw new Error(
      `Could not find a .usb-ui-test workspace from ${path.resolve(cwd)}. Pass --workspace <path> to target a UsbUiTest workspace explicitly.`,
    );
  }
...
export function resolveWorkspaceArtifactsRootDir(): string {
  return path.join(resolveUsbUiTestRootDir(), 'workspaces');
}
...
export async function loadWorkspaceConfig(usbUiTestDir: string): Promise<WorkspaceConfig> {
...
async function tryResolveWorkspace(
  cwd: string,
  options?: { requireSelectableWorkspace?: boolean },
): Promise<UsbUiTestWorkspace | undefined> {
  const workspaceRoot = await findWorkspaceRoot(
    cwd,
    options?.requireSelectableWorkspace ? isSelectableWorkspaceRoot : isWorkspaceRootCandidate,
  );
...
async function buildWorkspace(workspaceRoot: string): Promise<UsbUiTestWorkspace> {
...
function getWorkspaceMetadataPath(workspace: UsbUiTestWorkspace): string {
