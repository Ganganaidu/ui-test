...
function createTempResourceDir(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-cli-'));
  fs.mkdirSync(path.join(root, 'ios'), { recursive: true });
  fs.writeFileSync(path.join(root, 'ios', 'usb-ui-test-ios.zip'), 'zip');
  fs.writeFileSync(path.join(root, 'ios', 'usb-ui-test-ios-test-Runner.zip'), 'zip');
  return root;
}
  ...
  try {
    const filePathUtil = new CliFilePathUtil(
      ...
      (async (_file: string, args: readonly string[]): Promise<ExecResult> => {
        ...
        if (zipPath.endsWith('usb-ui-test-ios-test-Runner.zip')) {
          fs.mkdirSync(path.join(targetDir, 'usb-ui-test-ios-test-Runner.app'), {
            recursive: true,
          });
        } else if (zipPath.endsWith('usb-ui-test-ios.zip')) {
          fs.mkdirSync(path.join(targetDir, 'usb-ui-test-ios.app'), {
            recursive: true,
          });
        }
    ...
    assert.equal(
      ...
      path.join(
        ...
        'usb-ui-test-ios-test-Runner.app',
      ),
    );
    assert.equal(unzipCalls.length, 2);
    assert.deepEqual(
      unzipCalls.map((args) => path.basename(args[1] as string)),
      ['usb-ui-test-ios.zip', 'usb-ui-test-ios-test-Runner.zip'],
    );
  } finally {
...
test('CliFilePathUtil reuses pre-extracted iOS driver apps without unzipping again', async () => {
  ...
  fs.mkdirSync(path.join(targetDir, 'usb-ui-test-ios.app'), { recursive: true });
  fs.mkdirSync(path.join(targetDir, 'usb-ui-test-ios-test-Runner.app'), { recursive: true });
...
test('CliFilePathUtil downloads the Android driver asset from the manifest into the cache dir', async () => {
  const cacheDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-assets-cache-'));
  const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-assets-source-'));
