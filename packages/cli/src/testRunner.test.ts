...
function writeWorkspaceConfig(
  ...
  fs.mkdirSync(path.join(rootDir, '.usb-ui-test'), { recursive: true });
  fs.writeFileSync(path.join(rootDir, '.usb-ui-test', 'config.yaml'), `${lines.join('\n')}\n`, 'utf-8');
}
...
test('ReportWriter emits redacted JSON artifacts and input snapshots without persisted HTML', async () => {
  const runDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-report-'));
  const workspaceRoot = path.join(runDir, 'workspace');
  const testSourcePath = path.join(workspaceRoot, '.usb-ui-test', 'tests', 'auth', 'login.yaml');
  const envPath = path.join(workspaceRoot, '.usb-ui-test', 'env', 'staging.yaml');
...
test('ReportWriter persists suite snapshots and suite metadata without changing per-test result files', async () => {
  const runDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-suite-report-'));
  const workspaceRoot = path.join(runDir, 'workspace');
  ...
  const suiteSourcePath = path.join(workspaceRoot, '.usb-ui-test', 'suites', 'login_suite.yaml');
  const writer = new ReportWriter({
    runDir,
    envName: 'none',
    platform: 'android',
...
test('ReportWriter reuses artifact-local recording files without duplicating the copy step', async () => {
  const runDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-artifact-recording-'));
  const writer = new ReportWriter({
    runDir,
    envName: 'none',
    platform: 'android',
  ...
  const testDef: TestDefinition = {
    ...
    steps: ['Open login.'],
    expected_state: ['The dashboard is visible.'],
    sourcePath: path.join(runDir, 'workspace', '.usb-ui-test', 'tests', 'login.yaml'),
...
test('runTests finalizes top-level artifacts when shared-session execution throws before a test completes', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-runner-'));
...
test('runTests succeeds without env config when the repo is env-free', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-env-free-runner-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  fs.mkdirSync(testsDir, { recursive: true });
  fs.writeFileSync(
...
test('runTests records the suite subcommand in run metadata when invoked via usb-ui-test suite', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-suite-command-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  ...
  try {
    ...
    assert.equal(runJson.input.cli.command, 'usb-ui-test suite smoke.yaml');
...
test('runTests prepares one shared session for multiple tests and cleans it up once', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-shared-session-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests uses mov artifact recording output paths for iOS tests', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-ios-recording-output-'));
...
test('runTests stops the batch after a shared-session failure and cleans up once', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-shared-session-failure-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests stops remaining tests after a terminal AI provider failure', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-terminal-provider-failure-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests aborts the batch after SIGINT and marks the active run as aborted', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-shared-session-abort-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests requests a forced exit after a second SIGINT', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-shared-session-force-exit-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests requires base app config even when the env file contains an app override', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-validation-failure-'));
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
  fs.mkdirSync(envDir, { recursive: true });
...
test('runTests rejects validation failures before creating run artifacts', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-missing-selectors-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests surfaces device setup diagnostics before execution without creating run artifacts', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-setup-buffering-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests fails before prepareGoalSession when Android host preflight is blocked', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-android-preflight-failure-'));
  writeWorkspaceConfig(rootDir);
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.match(error.message, /usb-ui-test doctor --platform android/);
        return true;
      },
    );
...
test('runTests fails before prepareGoalSession when iOS host preflight is blocked', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-ios-preflight-failure-'));
  writeWorkspaceConfig(rootDir, 'ios');
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.match(error.message, /usb-ui-test doctor --platform ios/);
        return true;
      },
    );
    assert.equal(prepareCalls, 0);
...
test('runTests continues when one platform is healthy and the other is blocked', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-preflight-partial-'));
  writeWorkspaceConfig(rootDir, 'ios');
  const testsDir = path.join(rootDir, '.usb-ui-test', 'tests');
  const envDir = path.join(rootDir, '.usb-ui-test', 'env');
  fs.mkdirSync(testsDir, { recursive: true });
...
test('runTests requires --platform when both Android and iOS apps are configured', async () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-preflight-both-blocked-'));
