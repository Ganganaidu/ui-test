...
import type { RunManifest, RunTarget } from '@usb-ui-test/common';
...
test('rebuildRunIndex writes runs.json from run.json files', async () => {
  const artifactsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-run-index-'));
...
test('rebuildRunIndex carries compact suite target metadata into runs.json', async () => {
  const artifactsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'usb-ui-test-suite-run-index-'));
