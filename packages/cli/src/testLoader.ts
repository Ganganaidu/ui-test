...
export async function loadTest(
  ...
  let steps: string[];
  let structuredSteps: StructuredStep[] | undefined;
  if (mode === 'deterministic') {
    let parsedSteps: StructuredStep[];
    try {
      parsedSteps = parseStructuredSteps(rawSteps);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      throw new Error(`Test file ${filePath} steps: ${msg}`);
    }
...
export function validateTestBindings(
  ...
  if (unresolvedReferences.size > 0) {
    if (options?.environmentResolved === false) {
      throw new Error(
        `Test references environment bindings, but no environment configuration was resolved. Add .usb-ui-test/env/<name>.yaml or pass --env <name>: ${Array.from(unresolvedReferences).join(', ')}`,
      );
    }
    throw new Error(
 ...
 * an error in context).
 */
function readArrayOrEmpty(value: unknown, label: string): unknown[] {
...
function stringifyStructuredStep(step: StructuredStep): string {
  switch (step.kind) {
    case 'launchApp':
      return step.appId
...
function describeSelectorShort(sel: { text?: string; id?: string; accessibility?: string; contains?: string }): string {
  if (sel.text) return `"${sel.text}"`;
  if (sel.id) return `#${sel.id}`;
  if (sel.accessibility) return `@${sel.accessibility}`;
  if (sel.contains) return `~"${sel.contains}"`;
  return '<no-selector>';
}
