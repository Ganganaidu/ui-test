...
class ReportWriter {
    static _FAILURE_PLACEHOLDER_IMAGE = Buffer.from('', 'base64');
    _runDir;
    _envName;
    _platform;
    _runId;
    ...
    createLoggerSink() {
        return (entry) => {
            const renderedMessage = (0, common_1.redactResolvedValue)(entry.renderedMessage, this._bindings) ?? entry.renderedMessage;
            fs.appendFileSync(this._runnerLogPath, `${entry.timestamp} ${renderedMessage}\n`, 'utf-8');
        };
    }
    appendLogLine(line) {
        const renderedLine = (0, common_1.redactResolvedValue)(line, this._bindings) ?? line;
        fs.appendFileSync(this._runnerLogPath, `${new Date().toISOString()} ${renderedLine}\n`, 'utf-8');
    }
    appendRawBlock(block) {
        const renderedBlock = (0, common_1.redactResolvedValue)(block, this._bindings) ?? block;
        fs.appendFileSync(this._runnerLogPath, renderedBlock.endsWith('\n') ? renderedBlock : `${renderedBlock}\n`, 'utf-8');
    }
    ...
    async writeRunInputs(params) {
        ...
        await fsp.writeFile(path.join(this._runDir, this._runContextJsonPath), JSON.stringify({
            cli: params.cli,
            model: params.model,
        ...
        await fsp.writeFile(path.join(this._runDir, envSnapshotYamlPath), yaml_1.default.stringify({
            app: params.environment.config.app,
            secrets: params.environment.config.secrets,
            variables: params.environment.config.variables,
        }), 'utf-8');
        await fsp.writeFile(path.join(this._runDir, envSnapshotJsonPath), JSON.stringify(this._inputEnvironment, null, 2), 'utf-8');
        ...
        if (params.suite) {
            ...
            if (params.suite.sourcePath) {
                await fsp.copyFile(params.suite.sourcePath, path.join(this._runDir, suiteSnapshotYamlPath));
            }
            await fsp.writeFile(path.join(this._runDir, suiteSnapshotJsonPath), JSON.stringify(suiteRecord, null, 2), 'utf-8');
        ...
        for (const test of params.tests) {
            ...
            await fsp.writeFile(path.join(this._runDir, snapshotJsonPath), JSON.stringify({
                testId: test.testId,
                testName: test.name,
    ...
    async writeTestRecord(test, result, bindings) {
        ...
        for (const [index, step] of result.steps.entries()) {
            ...
            await fsp.writeFile(path.join(this._runDir, stepJsonRelative), JSON.stringify(artifactStep, null, 2), 'utf-8');
        }
        const testRecord = {
            testId: test.testId,
            testName: test.name,
            ...
            message: (0, common_1.redactResolvedValue)(result.message, bindings) ?? result.message,
            analysis: (0, common_1.redactResolvedValue)(result.analysis, bindings),
        ...
        await fsp.writeFile(path.join(testDir, 'result.json'), JSON.stringify(testRecord, null, 2), 'utf-8');
        return testRecord;
    }
    async finalize(params) {
        ...
        await fsp.writeFile(path.join(this._runDir, 'summary.json'), JSON.stringify(summary, null, 2), 'utf-8');
        await fsp.writeFile(path.join(this._runDir, 'run.json'), JSON.stringify(manifest, null, 2), 'utf-8');
        return summary;
    }
    async writeTestFailureRecord(params) {
        ...
        const screenshotRelative = path.posix.join('tests', params.test.testId, 'screenshots', '001.jpg');
        const failureMessage = (0, common_1.redactResolvedValue)(params.message, params.bindings) ?? params.message;
        const failureStep = {
            stepNumber: 1,
            iteration: 1,
            ...
            trace: {
                ...
                action: 'run_failure',
                status: 'failure',
                totalMs: Math.max(0, new Date(params.completedAt).getTime() - new Date(params.startedAt).getTime()),
                spans: [],
                failureReason: failureMessage,
            },
        };
        await fsp.writeFile(path.join(this._runDir, screenshotRelative), ReportWriter._FAILURE_PLACEHOLDER_IMAGE);
        await fsp.writeFile(path.join(this._runDir, stepJsonRelative), JSON.stringify(failureStep, null, 2), 'utf-8');
        const testRecord = {
    ...
    _buildRunManifest(params) {
        const testRecords = params.tests.map((test) => this._toRunManifestTest(test));
        const stepTotal = testRecords.reduce((total, test) => total + (test.counts?.executionStepsTotal ?? 0), 0);
        const stepPassed = testRecords.reduce((total, test) => total + (test.counts?.executionStepsPassed ?? 0), 0);
    ...
    _toRunManifestTest(test) {
        const snapshot = this._testSnapshots.get(test.testId);
        const steps = test.steps.map((step) => ({ ...step }));
        ...
        const firstFailure = firstFailureStep
            ? {
                ...
                message: firstFailureStep.errorMessage ??
    ...
    async _copyRecordingArtifact(testId, recording) {
        ...
        try {
            ...
            common_1.Logger.w(`Recording file not found for report copy: ${recording.filePath}`);
            return undefined;
        }
    ...
    async _copyLogArtifact(testId, deviceLog, bindings) {
        ...
        try {
            ...
            common_1.Logger.w(`Device log file not found for report copy: ${deviceLog.filePath}`);
            return undefined;
        }
        const stats = await fsp.stat(sourcePath);
        if (stats.size > 50 * 1024 * 1024) {
            common_1.Logger.w(`Device log file is large (${(stats.size / 1024 / 1024).toFixed(1)} MB): ${sourcePath}. Redaction will hold the entire file in memory.`);
        }
        await fsp.copyFile(sourcePath, targetPath);
        // Read, redact secrets, and write back
        try {
            const raw = await fsp.readFile(targetPath, 'utf-8');
            const redacted = (0, common_1.redactResolvedValue)(raw, bindings);
            if (redacted !== undefined && redacted !== raw) {
                await fsp.writeFile(targetPath, redacted, 'utf-8');
            }
        }
        catch (error) {
            common_1.Logger.w(`Failed to redact device log file: ${this._formatError(error)}`);
...
function resolveTestStatus(result) {
    if (result.status === 'aborted') {
        return 'aborted';
    }
    return result.success ? 'success' : 'failure';
}
function collectBindingReferences(test) {
...
function selectPreviewScreenshotPath(steps) {
...
function findRunFirstFailure(tests, diagnosticsSummary) {
...
function toAgentAction(step, params) {
    return {
        stepNumber: params.stepNumber,
        iteration: step.iteration,
        actionType: step.action,
        naturalLanguageAction: (0, common_1.redactResolvedValue)(step.naturalLanguageAction || step.reason, params.bindings) ?? step.naturalLanguageAction ?? step.reason,
        reason: (0, common_1.redactResolvedValue)(step.reason, params.bindings) ?? step.reason,
        analysis: (0, common_1.redactResolvedValue)(step.analysis, params.bindings),
        thought: step.thought
            ? {
                plan: (0, common_1.redactResolvedValue)(step.thought.plan, params.bindings),
                think: (0, common_1.redactResolvedValue)(step.thought.think, params.bindings),
                act: (0, common_1.redactResolvedValue)(step.thought.act, params.bindings),
            }
            : undefined,
        actionPayload: step.actionPayload
            ? {
                ...step.actionPayload,
                text: (0, common_1.redactResolvedValue)(step.actionPayload.text, params.bindings),
                url: (0, common_1.redactResolvedValue)(step.actionPayload.url, params.bindings),
            }
            : undefined,
        success: step.success,
        status: step.success ? 'success' : 'failure',
        errorMessage: (0, common_1.redactResolvedValue)(step.errorMessage, params.bindings),
...
function computeVideoOffsetMs(stepTimestamp, recordingStartedAt) {
...
function redactTrace(trace, bindings) {
    if (!trace) {
        return undefined;
    }
    return {
        ...trace,
        failureReason: (0, common_1.redactResolvedValue)(trace.failureReason, bindings),
        spans: trace.spans.map((span) => ({
            ...span,
            detail: (0, common_1.redactResolvedValue)(span.detail, bindings),
        })),
    };
}
function redactTiming(timing, bindings) {
    if (!timing) {
        return undefined;
    }
    return {
