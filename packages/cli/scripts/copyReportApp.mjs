...
// is expected to run `npm run build --workspace=@usb-ui-test/report-web` first.
...
try {
  ...
  console.error(
    `[cli:copyReportApp] Source not found: ${source}\n` +
      `Build the report-web SPA first: npm run build:app --workspace=@usb-ui-test/report-web`,
  );
  throw error;
}
