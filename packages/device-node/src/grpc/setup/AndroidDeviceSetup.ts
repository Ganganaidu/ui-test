...
export class AndroidDeviceSetup {
  ...
  async prepare(
    ...
    try {
      ...
      if (!driverPath) {
        throw new Error(
          'Driver app APK not found. Configure USB_UI_TEST_ASSET_DIR, USB_UI_TEST_ASSET_MANIFEST_PATH, or USB_UI_TEST_ASSET_MANIFEST_URL.',
        );
      }
      ...
      if (testAppPath) {
        ...
        const testInstalled = await this._adbClient.installTestApp(
          adbPath,
          deviceSerial,
          testAppPath,
        );
