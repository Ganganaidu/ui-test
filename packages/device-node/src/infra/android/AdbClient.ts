...
export const ANDROID_DRIVER_APP_PACKAGE_NAME = 'app.usbuitest.driver';
export const ANDROID_DRIVER_TEST_PACKAGE_NAME = 'app.usbuitest.driver.test';
...
export class AdbClient {
  ...
  async installTestApp(
    ...
    const result = await this._runAdb(
      adbPath,
      ['-s', deviceSerial, 'install', '-r', '-g', '-t', apkPath],
      `Failed to install test runner APK on ${deviceSerial}`,
    );
    if (result.success) {
      Logger.i(`Installed APK on ${deviceSerial}: ${apkPath}`);
    }
