#!/usr/bin/env node
// Assemble a per-platform runtime tarball that the UsbUiTest CLI extracts to
// ~/.usb-ui-test/runtime/<version>/ when local commands need their dependencies.
//
// Usage: node scripts/buildRuntimeTarball.mjs --target=<platform>
//   <platform> ∈ darwin-arm64 | darwin-x64 | linux-x64 | linux-arm64
//
// Output: packages/local-runtime/dist/usb-ui-test-runtime-<version>-<platform>.tar.gz
//
// The tarball layout:
//   manifest.json                # version, platform, file sha256 sums
//   install-resources/           # driver APKs (always) + iOS zips (darwin only)
//   proto/usbuitest/driver.proto  # gRPC schema
...
const tarballPath = resolve(
  dist,
  `usb-ui-test-runtime-${VERSION}-${target}.tar.gz`,
);
...
const iosAssets = ['ios/usb-ui-test-ios.zip', 'ios/usb-ui-test-ios-test-Runner.zip'];
...
// 2. Copy proto.
const protoSource = resolve(repoRoot, 'proto/usbuitest/driver.proto');
const protoTarget = resolve(stagingDir, 'proto/usbuitest/driver.proto');
...
if (!existsSync(reportAppSource)) {
  bail(`Missing report-web/dist/app at ${reportAppSource} — run \`npm run build --workspace=@usb-ui-test/report-web\` first.`);
}
