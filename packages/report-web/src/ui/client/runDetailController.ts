...
// `JSON.parse(document.getElementById('usb-ui-test-report-data').textContent)`.
//
// Public API:
...
function ensureRecordingControls(container: HTMLElement): void {
  ...
  const togglePlayback = async () => {
    // When a screenshot is currently shown without per-step video offsets,
    // hitting play swaps the shell over to video mode.
...
function syncRecording(container: HTMLElement, test: ReportPayloadTest, step: ReportPayloadStep): void {
  ...
  //      (this path stores screenshots without per-step timestamps).
