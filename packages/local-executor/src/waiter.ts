...
export async function waitUntilStable(
  fetchHierarchy: () => Promise<Hierarchy>,
  opts: StableOptions = {},
): Promise<Hierarchy> {
  ...
  let last: Hierarchy = await fetchHierarchy();
...
export async function retryUntil<T>(
  fetchHierarchy: () => Promise<Hierarchy>,
  predicate: (h: Hierarchy) => T | undefined,
  opts: WaitOptions = {},
): Promise<T> {
  const interval = opts.pollIntervalMs ?? 200;
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(
        `Timed out after ${opts.timeoutMs ?? 10000}ms${
 ...
 */
function fingerprint(hierarchy: Hierarchy): string {
  const parts: string[] = [];
  for (const n of hierarchy.flattenedHierarchy) {
    parts.push(
      `${n.id ?? ''}|${n.text ?? ''}|${n.accessibilityText ?? ''}|${
        n.bounds ? n.bounds.join(',') : ''
      }`,
    );
  }
  return parts.join('\n');
