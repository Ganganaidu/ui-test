...
const SWIPE_DIRECTIONS: ReadonlySet<SwipeDirection> = new Set([
  'up',
  'down',
  'left',
  'right',
 ...
 */
export function detectMode(steps: unknown[]): 'deterministic' | 'ai' {
  if (steps.length === 0) return 'ai';

  const isStructuredEntry = (entry: unknown): boolean => {
  ...
  const isStringEntry = (entry: unknown): boolean => typeof entry === 'string';

  const allStructured = steps.every(isStructuredEntry);
  if (allStructured) return 'deterministic';

  const allStrings = steps.every(isStringEntry);
  if (allStrings) return 'ai';

  // Mixed — find the first offender for a useful error message.
  for (let i = 0; i < steps.length; i++) {
    if (!isStructuredEntry(steps[i]) && !isStringEntry(steps[i])) {
      throw new Error(
        `step[${i}] is neither a structured command (e.g. {tapOn: ...}) nor a natural-language string`,
      );
    }
  }
  throw new Error(
    'steps mix natural-language strings and structured commands; pick one mode per test file',
  );
}
 ...
 */
export function parseStructuredSteps(
  rawSteps: unknown[],
): StructuredStep[] {
...
function parseOneStep(raw: unknown): StructuredStep {
  ...
  if (keys.length !== 1) {
    throw new Error(
      `expected exactly one command key, got ${keys.length} (${keys.join(', ')})`,
    );
  }
  const key = keys[0]!;
  const value = obj[key];
...
function parseLaunchApp(value: unknown): StructuredStep {
  ...
  throw new Error(
    'launchApp expected: empty | string appId | object {appId?, clearState?}',
  );
}

function parseClearAppData(value: unknown): StructuredStep {
  if (value === null || value === undefined) {
...
function parseInputText(value: unknown): StructuredStep {
  if (typeof value === 'string') {
...
function parseSwipe(value: unknown): StructuredStep {
  if (typeof value === 'string' && SWIPE_DIRECTIONS.has(value as SwipeDirection)) {
    return { kind: 'swipe', direction: value as SwipeDirection };
  }
  if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
    const obj = value as Record<string, unknown>;
    if (
      typeof obj['direction'] === 'string' &&
      SWIPE_DIRECTIONS.has(obj['direction'] as SwipeDirection)
    ) {
      return {
        kind: 'swipe',
        direction: obj['direction'] as SwipeDirection,
  ...
  throw new Error(
    'swipe expected: up|down|left|right | {direction} | {from:{x,y}, to:{x,y}, durationMs?}',
  );
}

function parseWaitFor(value: unknown): StructuredStep {
  if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
    const obj = value as Record<string, unknown>;
    return {
      kind: 'waitFor',
...
function parseSelector(value: unknown, ctx: string): Selector {
  if (typeof value === 'string') {
    return { text: value };
  }
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
  ...
  if (
    ...
    throw new Error(
      `${ctx} selector requires at least one of text/id/accessibility/contains`,
    );
  }
  return sel;
}
