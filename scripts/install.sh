...
#
# Env overrides:
#   USB_UI_TEST_DIR              Install root (default: $HOME/.usb-ui-test)
#   USB_UI_TEST_VERSION          Version to pin (default: latest GitHub release)
#   USB_UI_TEST_NON_INTERACTIVE  Set to skip all prompts (same as --ci)
...
GITHUB_REPO="gangadharkondati/usb-ui-test"
...
main() {
  ...
  if [ -n "${CI:-}" ] || [ -n "${USB_UI_TEST_NON_INTERACTIVE:-}" ]; then
    CI_MODE=true
  fi
  ...
  case "$os_raw" in
    ...
    CYGWIN*|MINGW*|MSYS*)
      ...
      info "Install on a macOS or Linux host, or run usb-ui-test in WSL2."
      exit 1
      ;;
  ...
  PLATFORM="${OS}-${ARCH}"
  USB_UI_TEST_DIR="${USB_UI_TEST_DIR:-$HOME/.usb-ui-test}"
  mkdir -p "$USB_UI_TEST_DIR/bin"

  VERSION=$(resolve_version)
  TAG="v${VERSION}"

  info "UsbUiTest Installer (${VERSION})"
  info "─────────────────────────────"
  echo ""
...
resolve_version() {
  if [ -n "${USB_UI_TEST_VERSION:-}" ]; then
    echo "$USB_UI_TEST_VERSION"
    return
  fi
  ...
  if [ -z "$redirect" ]; then
    fail "Could not resolve the latest usb-ui-test release. Set USB_UI_TEST_VERSION explicitly."
    exit 1
  fi
  if ! [[ "$redirect" =~ /releases/tag/v[0-9]+\.[0-9]+\.[0-9]+ ]]; then
    fail "Could not parse latest release tag from redirect URL:"
    fail "  $redirect"
    fail "Set USB_UI_TEST_VERSION explicitly to bypass auto-resolution."
    exit 1
  fi
...
install_binary() {
  local bin_url="https://github.com/${GITHUB_REPO}/releases/download/${TAG}/usb-ui-test-${PLATFORM}"
  BIN_DEST="$USB_UI_TEST_DIR/bin/usb-ui-test"
  local bin_tmp="${BIN_DEST}.tmp"

  info "Downloading usb-ui-test binary for ${PLATFORM}..."
...
setup_path() {
  local path_line="export PATH=\$PATH:$USB_UI_TEST_DIR/bin"
  local rc
  for rc in "$HOME/.bashrc" "$HOME/.bash_profile" "${ZDOTDIR:-$HOME}/.zshrc"; do
    touch "$rc" 2>/dev/null || true
    if [ -f "$rc" ] && ! grep -qF "$USB_UI_TEST_DIR/bin" "$rc" 2>/dev/null; then
      echo "$path_line" >> "$rc"
    fi
  done
}
...
print_ci_summary() {
  echo ""
  ok "usb-ui-test installed."
  echo ""
  info "For local test execution on this machine, re-run without --ci:"
  echo ""
  echo "    curl -fsSL https://raw.githubusercontent.com/${GITHUB_REPO}/main/scripts/install.sh | bash"
  echo ""
  echo "Open a new terminal or run:  export PATH=\"\$PATH:$USB_UI_TEST_DIR/bin\""
}
...
download_runtime() {
  ...
  local runtime_url="https://github.com/${GITHUB_REPO}/releases/download/${TAG}/usb-ui-test-runtime-${VERSION}-${PLATFORM}.tar.gz"
  RUNTIME_DIR="$USB_UI_TEST_DIR/runtime/${VERSION}"
  local runtime_tmp="$USB_UI_TEST_DIR/runtime/${VERSION}.tmp"
  local tar_path="$USB_UI_TEST_DIR/runtime/${VERSION}.tar.gz"

  mkdir -p "$USB_UI_TEST_DIR/runtime"
  rm -rf "$runtime_tmp"
  mkdir -p "$runtime_tmp"
...
prompt_platform() {
  ...
  if [ -z "$PLATFORM_CHOICE" ] && [ $attempts -ge 3 ]; then
    ...
    warn "Re-run the installer to try again, or run 'usb-ui-test doctor' to diagnose host tooling."
  fi
}
...
prompt_skills() {
  echo ""
  info "── UsbUiTest AI Agent Skills ──"
  echo ""
  printf "Install AI agent skills (used by Claude Code/Cursor for /usb-ui-test-* commands)? [Y/n] [30s timeout]: "
  local reply choice
  if read -r -t 30 reply; then
    case "$reply" in
  ...
  if [ "$choice" = "install" ]; then
    if ! command -v npx >/dev/null 2>&1; then
      ...
      info "Installing UsbUiTest skills..."
      npx skills add gangadharkondati/usb-ui-test && ok "UsbUiTest skills installed."
    fi
  fi
}
...
print_summary() {
  ...
  info "── Summary ──"
  echo ""
  ok "usb-ui-test ${VERSION} installed at $BIN_DEST"
  ...
  echo ""
  echo "    export PATH=\"\$PATH:$USB_UI_TEST_DIR/bin\""
  echo ""
  info "Try it:  usb-ui-test --help"
  echo ""
}
