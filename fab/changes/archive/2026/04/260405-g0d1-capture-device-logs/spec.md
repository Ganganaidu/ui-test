...
- **GIVEN** a consumer imports from `@usb-ui-test/common`
...
Log files SHALL be written to `<tmpDir>/usb-ui-test-logs/{sanitizedRunId}_{sanitizedTestId}.log`. The `LogCaptureManager` SHALL NOT write directly into the report directory.
...
3. Reads the copied file, applies `redactResolvedValue()` from `@usb-ui-test/common`, and writes back
