---
title: "Inject secrets and variables into UsbUiTest test specs"
sidebarTitle: "Placeholders"
description: "Learn how to use ${secrets.*} and ${variables.*} placeholders in UsbUiTest test specs, how they resolve at run time, and how to configure env files safely."
---

UsbUiTest test specs support two types of placeholders that let you inject dynamic values without hardcoding them in YAML files. You reference a placeholder in a step; UsbUiTest resolves its value at run time from environment variables or a binding file. This keeps credentials and configuration out of your test source.
...
Both types must be declared in `.usb-ui-test/env/<name>.yaml` before you use them in a test.

## Env binding file

Create a file under `.usb-ui-test/env/` for each environment you use (e.g. `dev.yaml`, `staging.yaml`). Declare secrets as `${SHELL_ENV_VAR}` placeholders and variables as plain values:
...
When you run UsbUiTest with an env named `N` (e.g. `--env dev`), the CLI resolves values in this order:
...
Pass `--env <name>` to the CLI to activate a specific env profile. The name must match a file under `.usb-ui-test/env/`:

```bash
usb-ui-test test auth/login.yaml --platform android --model google/gemini-3-flash-preview --env staging
```

UsbUiTest will load `.env.staging`, then `.env`, then `process.env`, and resolve all `${secrets.*}` bindings declared in `.usb-ui-test/env/staging.yaml`.
