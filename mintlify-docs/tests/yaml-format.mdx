---
title: "UsbUiTest YAML test format: fields, phases, examples"
sidebarTitle: "YAML Format"
description: "Learn the UsbUiTest YAML test format: required fields, the three-phase execution model, allowed actions, and how to write reliable natural-language steps."
---

UsbUiTest test specs are plain YAML files stored under `.usb-ui-test/tests/`. Each file defines a single test scenario using natural-language steps that the AI agent executes on a real device or emulator. You describe what a user would do; UsbUiTest taps, swipes, types, and verifies on your behalf.
...
<Note>
  Group tests by feature under `.usb-ui-test/tests/<feature>/`. For example, authentication tests belong in `.usb-ui-test/tests/auth/`, and onboarding tests in `.usb-ui-test/tests/onboarding/`. This mirrors the suite structure and makes it easy to run all tests for a given feature at once.
</Note>
