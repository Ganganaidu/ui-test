---
title: "UsbUiTest test reports: browse artifacts, video, and logs"
sidebarTitle: "Reports"
description: "Explore UsbUiTest run artifacts, open the local report UI, read result.json, and inspect failures with video playback and device logs."
---

After every test run, UsbUiTest saves a full set of artifacts to disk so you can diagnose failures, replay executions, and track results over time. You can browse these artifacts directly on the filesystem or through the local report UI, which adds video playback, log search, and step-by-step screenshot browsing.
...
Use `usb-ui-test runs` to list local reports for the current workspace:
...
UsbUiTest starts a local web server (or reuses an existing one) and opens the report UI in your browser. The UI shows all runs for the current workspace.

<Tip>
  Run `usb-ui-test start-server` immediately after a test failure to replay the recording, step through screenshots, and search device logs — all in sync. This is the fastest way to understand what the AI agent saw on screen at each step.
</Tip>
...
<Note>
  When the `/usb-ui-test-use-cli` or `/usb-ui-test-test-and-fix` skill diagnoses a failure, it reads `result.json`, the matching `actions/` entry, and the screenshot at the failed step before suggesting any fix. You can do the same inspection manually using `usb-ui-test start-server`.
</Note>
