...
- A small `usb-ui-test` program for each of: macOS Apple Silicon, macOS Intel, Linux x86_64, Linux ARM64.
...
Users install the `usb-ui-test` program by running:
...
npm version X.Y.Z -w @usb-ui-test/cli --no-git-tag-version
```
...
Or open `https://github.com/gangadharkondati/usb-ui-test/releases/tag/vX.Y.Z` in a browser.
...
That's it — `usb-ui-test upgrade` on user machines, and fresh `curl ... | bash` runs, will now pull your new version.
...
for t in darwin-arm64 darwin-x64 linux-x64 linux-arm64; do
  npm run build:tarball --workspace=@usb-ui-test/local-runtime -- --target=$t
done
...
gh release create "v$VERSION" \
  --title "UsbUiTest $VERSION" \
  --notes-file /tmp/release-body.md \
  --latest \
  dist/binaries/usb-ui-test-* \
  packages/local-runtime/dist/usb-ui-test-runtime-*.tar.gz*
```
...
Note: anyone who already installed the broken version still has it on their disk. They'll get the new version when they run `usb-ui-test upgrade` or re-run the curl install command. The public install URL goes through "latest", so deleting the broken release immediately stops new users from getting it.
