...
UsbUiTest workspaces are configured through `.usb-ui-test/config.yaml` and optional per-environment files under `.usb-ui-test/env/`.
...
When neither workspace `reasoning:` nor a per-feature `reasoning:` is set, UsbUiTest applies built-in fallbacks:
...
`anthropic/...` models must be Claude 4.5 or later (Sonnet 4.5+, Opus 4.5+, Haiku 4.5+, including Sonnet 4.6, Opus 4.6, and Opus 4.7). UsbUiTest uses Anthropic's native structured-output API (`output_config.format`) for guaranteed JSON, and only Claude 4.5+ supports it. Older Anthropic models will return HTTP 400 from the API. OpenAI and Google paths have no equivalent restriction.
...
UsbUiTest needs to know which app to launch on the device. The app identity is resolved in this order:

1. **`--app <path>`** CLI flag (highest priority)
2. **`.usb-ui-test/env/<name>.yaml`** `app` block (per-environment override)
3. **`.usb-ui-test/config.yaml`** `app` block (workspace default)
...
If your app uses different identifiers per environment (e.g. `.staging` suffix), override the app identity in `.usb-ui-test/env/<name>.yaml`:

```yaml
# .usb-ui-test/env/staging.yaml
app:
  packageName: com.example.myapp.staging
  bundleId: com.example.myapp.staging
```

The default identity in `.usb-ui-test/config.yaml` is used for any environment that doesn't define its own `app` block.

## Environment Bindings

Environment files (`.usb-ui-test/env/<name>.yaml`) can contain:
