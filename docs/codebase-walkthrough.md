# UsbUiTest Codebase Walkthrough

A complete guide to understanding how UsbUiTest works, from CLI invocation to test execution to report display.
...
UsbUiTest is a **monorepo with 5 packages** that together form an AI-powered mobile app testing tool:
...
User writes YAML test     CLI parses & validates     Device is prepared
  (.usb-ui-test/tests/)   ──────────────────────────>  (connect, install, launch)
                                                           │
                                                           v
...
### 2.1 Test YAML (`.usb-ui-test/tests/auth/login.yaml`)
...
### 2.2 Suite YAML (`.usb-ui-test/suites/smoke.yaml`)

```yaml
name: Smoke Suite
description: Quick validation of critical paths

tests:                              # References to test files (relative to .usb-ui-test/tests/)
  - auth/login.yaml
  - auth/signup.yaml
  - checkout/guest-checkout.yaml
```

**Why suites?** Running `usb-ui-test test auth/login.yaml auth/signup.yaml checkout/guest-checkout.yaml` every time is tedious. A suite groups related tests and gives them a name. The suite's `tests:` array contains **selectors** — file paths or glob patterns that resolve to test files.

**Scenario: Suite vs Direct**
```bash
# Direct: run specific tests
usb-ui-test test auth/login.yaml checkout/guest-checkout.yaml

# Suite: run a named group
usb-ui-test suite smoke.yaml
```

Both paths converge to the same execution pipeline — the only difference is how tests are selected.

### 2.3 Environment YAML (`.usb-ui-test/env/dev.yaml`)
...
secrets:                            # Values read from actual environment variables
  email: ${USB_UI_TEST_TEST_EMAIL}     # Must be ${ENV_VAR_NAME} format exactly
  password: ${USB_UI_TEST_TEST_PASSWORD}
...
**Why separate from tests?** Secrets should never be in YAML files. The `${USB_UI_TEST_TEST_EMAIL}` syntax is a **placeholder** — at runtime, UsbUiTest reads the actual value from the process environment. This means:
...
### 2.4 Workspace Config (`.usb-ui-test/config.yaml`)
...
**Why?** This is the "base" app identity. Environment configs can override it (e.g., `org.wikipedia.beta` for dev). At least one of `packageName` or `bundleId` is required so UsbUiTest knows which app to test.

---

## 3. CLI Entry Point & Command Dispatch

**File:** `packages/cli/bin/usb-ui-test.ts`

The CLI uses `commander` for argument parsing. Here are the key commands:

```text
usb-ui-test test [selectors...]     Run tests directly
usb-ui-test suite <path>            Run a suite
usb-ui-test check [selectors...]    Validate without executing
usb-ui-test runs                    List past runs
usb-ui-test start-server            Start the report server
usb-ui-test doctor                  Check host readiness
```

### The `test` command flow

```bash
usb-ui-test test auth/login.yaml --env staging --platform android --model openai/gpt-5.4-mini
```

**What happens in `runTestCommand()` (bin/usb-ui-test.ts:271-350):**

```text
1. normalizeTestSelectors()     Split comma-separated selectors, trim whitespace
2. resolveWorkspace()           Walk up directory tree to find .usb-ui-test/
3. loadWorkspaceConfig()        Read .usb-ui-test/config.yaml
4. parseModel()                 Parse "openai/gpt-5.4-mini" → { provider: "openai", modelName: "gpt-5.4-mini" }
5. resolveEnvironmentFile()     Find the right .usb-ui-test/env/*.yaml
...
Before touching any device, UsbUiTest validates everything:
...
runCheck()
  ...
  │   ├── Match against all .yaml files in .usb-ui-test/tests/
...
prepareTestSession()
  ...
  │   │   ├── Install driver APK (app.usbuitest.android)
  │   │   ├── Install test runner APK (app.usbuitest.android.test)
...
**Why gRPC?** UsbUiTest installs a small driver app on the device that exposes a gRPC server. The host machine connects to it via port forwarding. gRPC is used because:
...
During UiAutomation readiness polling, the driver may return "Already connected" — this is a **transient** state during Android's UiAutomation framework initialization. UsbUiTest classifies this as retryable (see `TRANSIENT_CAPTURE_PATTERNS` in `ScreenshotCapture.ts`).
...
Model and reasoning effort are configurable per feature (planner, grounder, and the specialized grounders) via the `features:` block in `.usb-ui-test/config.yaml`. See [configuration.md](configuration.md) for the YAML shape.
...
UsbUiTest has two ways to display reports:
...
| **When used** | Default (`usb-ui-test start-server`) | Development or custom deploy |
| **Routes** | Same | Same |
...
After test completes:
  ...
  │   ├── Spawn `usb-ui-test internal-report-server` as detached process
...
**Why detached process?** The report server must outlive the CLI process. After `usb-ui-test test` exits, the user should still be able to browse the report.
...
┌─────────────────────────────────────────────────────────────────────┐
│                         USER INPUT                                  │
│  .usb-ui-test/config.yaml    .usb-ui-test/tests/*.yaml    .usb-ui-test/env/  │
│  .usb-ui-test/suites/*.yaml                                            │
└────────────────────────────────┬────────────────────────────────────┘
                                 │
                                 v
┌─────────────────────────────────────────────────────────────────────┐
...
```json
{
  ...
  "input": {
    "environment": {
      ...
      "secretReferences": [
        { "key": "email", "envVar": "USB_UI_TEST_TEST_EMAIL" }
      ]
    },
    "suite": {
      "suiteId": "smoke",
      "name": "Smoke Suite",
      "workspaceSourcePath": ".usb-ui-test/suites/smoke.yaml",
    ...
    "tests": [
      {
        ...
        "workspaceSourcePath": ".usb-ui-test/tests/auth/login.yaml",
    ...
    "cli": {
      "command": "usb-ui-test suite smoke.yaml --env dev",
      "selectors": [],
      "suitePath": "smoke.yaml",
      "debug": false
    }
  ...
  "tests": [
    {
      "testId": "auth-login",
      "testName": "User Login Flow",
      "sourcePath": "/repo/.usb-ui-test/tests/auth/login.yaml",
      "relativePath": "auth/login.yaml",
      "success": false,
      "status": "failure",
      "message": "Login button not found after entering credentials",
...
|------|-------------|---------------|
| `bin/usb-ui-test.ts` | CLI entry point, command definitions | `runTestCommand()` |
...
```bash
USB_UI_TEST_TEST_EMAIL="user@test.com" usb-ui-test suite smoke.yaml --env dev
```

1. **CLI parses** `suite smoke.yaml --env dev` → dispatches to `runTestCommand()`
2. **Workspace found**: walks up to find `.usb-ui-test/` directory
3. **Config loaded**: reads `.usb-ui-test/config.yaml` → gets app identity (`org.wikipedia`)
4. **Environment loaded**: reads `.usb-ui-test/env/dev.yaml` → resolves `${USB_UI_TEST_TEST_EMAIL}` → `"user@test.com"`
5. **Suite loaded**: reads `.usb-ui-test/suites/smoke.yaml` → gets test list `["auth/login.yaml", "checkout/guest.yaml"]`
