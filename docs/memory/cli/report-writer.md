...
5. **Write-then-redact**: reads the copied file back, applies `redactResolvedValue(raw, bindings)` from `@usb-ui-test/common/repoPlaceholders`, writes redacted content back. Only writes if redaction actually changed the content.
