---
name: usb-ui-test-test-and-fix
description: Orchestrate the full UsbUiTest loop — use usb-ui-test-generate-test to explore a feature and author tests, run them via usb-ui-test-use-cli, then triage failures from CLI artifacts and fix app code or tests until the run is green. Trigger for requests like "debug this failing UsbUiTest test", "verify and fix the feature with UsbUiTest", or as the Definition-of-Done loop after a UI change.
---

# UsbUiTest Test and Fix Orchestrator

You own the end-to-end **generate → run → diagnose → fix** loop for UsbUiTest coverage in this repository. You do **not** replace `usb-ui-test-generate-test` or `usb-ui-test-use-cli` — you call into them. Your job is to keep the loop moving: plan tests for the feature, execute them, read the artifacts on failure, decide whether the bug is in the **app code** or the **test**, apply the narrowest fix, and re-run until green or until you hit a legitimate blocker.
...
- **Suspected bugs found while exploring are hypotheses, not fix targets.** While `usb-ui-test-generate-test` reads source code to plan tests, you may notice code that looks broken or inconsistent with the acceptance criteria. **Do not fix it yet.** Note it as a hypothesis, make sure the generated test would actually exercise that path, and let the UsbUiTest run confirm or refute it. Fixing source code before the test runs hides which behaviors the test actually catches and risks "fixing" code that was fine.
- **Generate and run before fixing.** The order is strict: first author or update tests via `usb-ui-test-generate-test`, then get `usb-ui-test check` clean, then execute via `usb-ui-test-use-cli`, and only after that read artifacts and apply fixes. Do not edit app code before the test has run, even if the hypothesis from exploration feels obvious.
...
- **Never fabricate secrets, credentials, or env values.** If a run blocks on a missing shell variable or missing `.usb-ui-test/env/<env>.yaml` binding, hand off to the user with the exact variable name and command. Do not invent values.
...
Invoke **`usb-ui-test-generate-test`**. Feed it the current session's plan, acceptance criteria, touched files, and described user flows so its Steps 1–4 build on existing context. Let that skill own the YAML authoring rules, folder grouping, env file handling, and `usb-ui-test check` loop.
...
Return here only after `usb-ui-test check` is clean on the scope you changed.

### 2. Run

Invoke **`usb-ui-test-use-cli`** to execute the affected scope. Follow that skill's command construction — it owns the flag semantics. In short:
...
- **Default to `usb-ui-test test <selectors>` against the single spec(s) you just touched** so iteration is fast and the first failure surfaces immediately. Do **not** run `usb-ui-test suite` unless the user explicitly asks for a suite-level run (for example, to confirm no regressions across the whole feature before sign-off).
- Pass `--env <name>` when the workspace has multiple `.usb-ui-test/env/*.yaml` files.
...
If the run is red, read the artifacts the CLI listed. The layout and reading order are defined in `usb-ui-test-use-cli` under **Post-Execution → On failure** — use that list verbatim rather than duplicating it here. Before writing anything to the user, you should have read at minimum `result.json` for the failing test and the `actions/` entry for the step that failed, plus the screenshot at that step.
...
- **Test YAML fixes:** if the change is a small wording tweak in an existing step or `expected_state` entry, edit the YAML in place. If the change is non-trivial (new setup steps, new edge-case coverage, env binding changes, new feature folder), route back to **`usb-ui-test-generate-test`** so its rules (idempotent setup, allowed action vocabulary, positional-context guidance, env file shape) are applied consistently.
...
Re-run **`usb-ui-test check`** on the changed scope. If app code changed, ask the user whether to rebuild before re-running — do not rebuild silently. When they confirm a rebuild, pass the new `--app` artifact; otherwise reuse the existing build path. Then re-run the same `usb-ui-test test` command that failed (stay on the single-spec, fail-fast path; do not escalate to `usb-ui-test suite` unless the user asks). Loop back to step 3 until green.
...
- Authoring YAML from scratch, deciding folder structure, or editing `.usb-ui-test/env/*.yaml` binding shape → route to **`usb-ui-test-generate-test`**.
- Teaching CLI flags, artifact layout, host readiness, provider/model selection, or `usb-ui-test doctor` / `usb-ui-test runs` / `usb-ui-test start-server` usage → route to **`usb-ui-test-use-cli`**.
...
- Any request that is purely about **writing or updating** YAML tests, suites, or env bindings → `usb-ui-test-generate-test`.
- Any request that is purely about **running, validating, or reading artifacts** from the CLI → `usb-ui-test-use-cli`.
- Requests to **debug a failing UsbUiTest run, verify a feature end-to-end, or close out a UI change with UsbUiTest coverage** → this skill, which will call the other two in order.
