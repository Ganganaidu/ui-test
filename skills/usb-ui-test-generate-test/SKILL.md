---
name: usb-ui-test-generate-test
description: Generate test and suite specifications in the strict UsbUiTest YAML format. Handles automated test planning, folder grouping by feature, repo app configuration, environment-specific overrides in .usb-ui-test/env/*.yaml, and validation via usb-ui-test check.
---

# UsbUiTest Test and Suite Generator

You are an expert QA Automation Engineer. Generate UsbUiTest YAML artifacts with extreme precision. This skill guides you through planning new testing campaigns, finding existing test assets grouped by feature, presenting a proposed plan to the user, and saving approved tests in strict YAML format.
...
- **Prerequisite:** The **`usb-ui-test` CLI** must be installed and available on `PATH` (the same binary the user will use to run tests). Declarations in `.usb-ui-test/env/<env>.yaml` are validated and resolved by **`usb-ui-test check`** and at run time; without the CLI you cannot confirm bindings are correct.
...
- Use `${variables.KEY}` and `${secrets.KEY}` in tests. **Both** must be **declared** under `variables` and `secrets` in `.usb-ui-test/env/<env>.yaml` (one file per environment name).
- **Secrets in YAML** use the UsbUiTest form `secrets.logical_key: "${SHELL_ENV_VAR}"` (placeholder only). Real values are supplied by the **shell or CI environment** at `usb-ui-test check` and run time.
- **Do not** assert or require that `.usb-ui-test/.env.*` files exist; do not treat secret storage files as part of validation. Do not read or write `.env.*` files.

**App configuration (required):**
- UsbUiTest runs require `.usb-ui-test/config.yaml` to define the default app identity for the repo.
...
- If the identifier is the same everywhere, keep app identity only in `.usb-ui-test/config.yaml`.
- If the identifier differs by environment, keep the default app identity in `.usb-ui-test/config.yaml` and replace it with a full env-specific `app` block under `.usb-ui-test/env/<env>.yaml`.
## Workflow Steps
...
- **Inference rules:**
  - If one identifier is clearly the repo default, write it to `.usb-ui-test/config.yaml`.
  - If env-specific identifiers are clearly derivable, keep the default in `.usb-ui-test/config.yaml` and put only the differing value in `.usb-ui-test/env/<env>.yaml`.
  - Reuse existing env names from `.usb-ui-test/env/*.yaml` before inventing new ones.
- **Guardrails:**
  ...
  - Do **not** silently overwrite an existing `.usb-ui-test/config.yaml` app block. Show the proposed change first.
...
- **Inspect:** Read `.usb-ui-test/config.yaml` and `.usb-ui-test/env/*.yaml` if present so you reuse the existing app config and binding keys.
...
- **App setup:**
  - Ensure `.usb-ui-test/config.yaml` has the default repo app identity.
  ...
  - If yes, add only the env-specific `app` override to `.usb-ui-test/env/<env>.yaml`.
  - If no, keep the app identity only in `.usb-ui-test/config.yaml`.
...
- **Which env files to update:** **Ask the user.** Default recommendation: add the same keys to **every** `.usb-ui-test/env/*.yaml` so all environments stay aligned unless they explicitly want a subset.
- **Allowed env file shapes:** `.usb-ui-test/env/<env>.yaml` may contain `app`, `variables`, `secrets`, or any combination of them.
...
1. Inspect the `.usb-ui-test/tests/` directory. Tests are grouped into sub-folders matching feature names (e.g., `.usb-ui-test/tests/<feature-name>/`).
2. Search for a feature folder relevant to the current request.
   ...
   - **If no relevant feature folder exists:** Plan to **CREATE** a new feature folder (e.g., `.usb-ui-test/tests/<new_feature_name>/`).
...
3. Repeat a similar process for test suites under `.usb-ui-test/suites/`. Suite files should match the feature folder conceptually, typically `.usb-ui-test/suites/<feature-name>.yaml`. Update an existing suite to include the new test(s) or create a new suite if none exists.
...
- **Setup checklist:** List every `${variables.*}` and `${secrets.*}` the tests will use, and confirm the matching entries you will add to `.usb-ui-test/env/*.yaml` (secret rows as `${ENV_VAR}` only).
- **Effective app checklist:** State which app identifier UsbUiTest should use for each env/platform affected by the change.
...
### Step 5 — Generate UsbUiTest artifacts
Once approved, generate or update tests and suites using strict UsbUiTest YAML syntax.
- **Variable and secret declarations:** Whenever the tests reference `${variables.*}` or `${secrets.*}`, create or update the corresponding `.usb-ui-test/env/*.yaml` files in the same change set (per user choice: all env files or a subset).

### Step 6 — Validation
**Source of truth:** run **`usb-ui-test check`** (with `--suite` or the right selectors and `--env` when multiple env files exist). That command validates the workspace, resolves bindings, and surfaces missing env vars or unknown keys.
- Do not rely on ad-hoc greps alone for binding correctness; use `usb-ui-test check` outcomes as the acceptance bar.
...
### Test File Template (`.usb-ui-test/tests/<feature-name>/<file>.yaml`)
...
### Suite File Template (`.usb-ui-test/suites/<feature-name>.yaml`)
...
tests:
  - .usb-ui-test/tests/<feature-name>/<file>.yaml
```
...
1. **Run `usb-ui-test check`** on the same scope you changed. Use **`--suite <path>`** under `.usb-ui-test/suites` when validating a suite, **or** pass **positional selectors** (YAML paths, directories, or globs under `.usb-ui-test/tests/`). Pass **`--env <name>`** when the workspace has multiple `.usb-ui-test/env/*.yaml` files and the CLI requires a choice, or when validating a specific named profile.
2. **If `usb-ui-test check` fails,** treat the command output as the source of truth. Typical fixes: missing or mistyped **`variables.*`** / **`secrets.*`** in `.usb-ui-test/env/<env>.yaml` (one file per environment name, e.g. `dev.yaml`, not named after a shell variable), references in specs that do not match declared keys, or required **`secrets.*`** values not exported in the shell/CI environment (placeholders in YAML must stay as `"${SHELL_ENV_VAR}"`; never commit plaintext secrets).
...
4. **Re-run `usb-ui-test check`** until it succeeds. If errors point at spec structure or paths, fix those YAML files and check again.

If `usb-ui-test` is not on `PATH`, resolve the install or invoke it by absolute path before relying on check output.
